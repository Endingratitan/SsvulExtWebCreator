...
- 构建开关（都可裸写，排错用）：`--rebuild`（忽略 `.ssvul/deps-*.json`，全量重写）、`--detect`（构建后打印变更清单）、`--git` / `--no-git`（强制变更检测器）、`--git-init 0|1`（是否允许在 `sets/` 建 git 基线）、`--threads auto|0|N`（写盘并发度）
...
| 页面渲染 | O(页数 × 每页 md/div 工作量) | **全量重渲染**（增量留 v5）：每条 md 从零解析、div 模板逐页替换 |
| 全局聚合 | O(唯聚合数 × 内容长度) | origin 集去重 → 文件级内容去重 → JsDeduper/CssDeduper → 压缩 |
| 写盘 | O(写集大小) | 内容未变的产物**跳过写盘**（写集仍完整记录，供孤儿清理/增量用） |
| 变更检测（可选） | O(变更数) 或 O(源文件数) | 只在 `--detect`／预览时跑：有 git 仓库用 `git status`，否则 stat 快筛 |
...
**实测（本机 WSL + `/mnt/d`，同一批源连跑取稳定值）**：example-sets（12 页）冷 3.4–4.2s / 热 **1.8–2.0s**；300 页合成站 冷 6.9–8.3s / 热 **5.5–6.0s**。热构建的构成大致是：扫描 0.4–1.9s + 页面 0.6–3.1s + 写盘 0.45–0.85s（写盘这部分主要是"每个产物一次 stat"的快速校验，属 `rsync` 式 size+mtime 判断）。

> ⚠️ **诚实预期**：管线是**全量重渲染**，0.3.3 砍掉的是"每页固定开销"与重复写盘；"存盘即见 ≤200ms"要等 **v5 增量重建**（依赖 `.ssvul/deps.json` 里的源→产物图）。
...
- 监听是**轮询**（本机 9p 上 inotify 完全无事件）：判据与构建**同一套**（有 git 仓库用 `git status`，否则 stat 快筛 + **内容哈希终判**）→ 纯 `touch` 不误报、"同长度改写"也不漏判。
...
- 源被删除/改名后旧产物的**自动清理**还没做（后续版本），但预览会给出警告——否则那些旧文件会一直被服务，看起来像"改了没生效"。
...
| `.ssvul/deps-<站点号>.json` | 每次构建（原子写；**内容未变就不写**） | 依赖/状态记录：源的 size+mtime、产物的 size+mtime+sha256 → 二次构建跳过写盘；将来的孤儿清理与增量重建的前提。**每个站点一份**（站点号 = sets 与 output 绝对路径的哈希），同一父目录下构建多个站点不会互相顶掉。**不在 `output/` 里**，不会被打包部署 |
| `sets/.git/`（可选） | 首次构建（标准布局 + git 可用时） | 站点源自己的仓库：变更检测走 `git status`（300 文件 ~340ms，比 stat 遍历 ~530ms 更快，且**纯 touch 不误报**） |

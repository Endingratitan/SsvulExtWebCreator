...
- 串联即级联：子 div 的 css 排在父后面 → **同选择器后到即胜**：构建期 CssDeduper 会把**前一条同选择器规则整块删除**。这是省流量的**既定特性，不是等价变换**。
  - ⚠️ **同一份 css 文件内部不互删**（2026-09 修正）：一个文件里把同名选择器分两次写（基础样式 + 后续微调）是正常写法，两条都会保留；只有"后一个来源文件"才会删掉更早文件里的前驱。
...
> **增量语义（0.4.0）**：上面这几步**只对本次真的被重渲染的页面生效**——依赖未变的页会整体跳过、直接沿用已有产物。
> 所以"改一个 div 的 css → 只有用到它的页重新走一遍去重/压缩"是预期行为，且产物字节与全量构建**逐字节相同**（门禁钉住）。
